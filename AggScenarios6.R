...
AggScenarios6 <- function (f, fname) {
  ...
  # 1- carMiles 
  f.carMiles  <- sqldf ('SELECT f.ID,f.HHoldGOR_B02ID, sum(f.TripDisIncSW) FROM f WHERE (f.MainMode_B04ID IN (3,4,5,12) 
  ...
  colnames(f.carMiles)[3] <- fname #rename column
  
  carMiles <<- cbind(carMiles, f.carMiles[,3])
  ...
  CO2.Tm <<- cbind(CO2.Tm, 1.61 * (3.1 / 1.61) * 1e-1 * f.carMiles[,3])    #this is KG !! No TDR as it's been 
  #already applied
