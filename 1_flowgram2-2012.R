...
flowgram <-function(MS1,TDR1,ebikes1,equity1) {
...
newtime <- TDR1 * distance/ speed       #for normal bike, initially

######   ALREADY CYCLED TRIPS, just get MMET  
     
if (baseline[k,'Cycled']==1)  {  #already cycling, TDR applied > Pcyc=1 + get MMETh
     baseline[k,'Pcyc']<- p <- 1
     ...
     baseline[k,'MMETh']= (METcycling-1) * newtime
         ...
         if (baseline[k,'cyclist']==0) {       #NOT CYCLISTS PEOPLE
              baseline[k,'TripTotalTime1'] = TDR1 * oldtime }     
        
         else if (baseline[k,'cyclist']==1) {   #CYCLISTS          *****
              #CALC p no matter what TDR
              p = pcyc21(age,sex,distance,ebikes1,equity1,MS1,TDR1)      
...
###### ANALYSIS OF CASES: normal bikes | ebikes | cycled or not (inc. walked)
                    ...
                    choice <-bikechoice(distance,TDR1)     
                                        
                    if (choice==1)   {     #user goes for ebike
                         #ebike speed calc.
                         baseline[k,'ebike']= 1
                         speed <- tripspeed(age,sex,1)
                         newtime = TDR1 * distance/ speed
              ...
              baseline[k,'TripTotalTime1']= TDR1 * oldtime
              baseline[k,'METh']= TDR1 * baseline[k,'METh']
              baseline[k,'MMETh']= TDR1 * baseline[k,'MMETh']
              }
